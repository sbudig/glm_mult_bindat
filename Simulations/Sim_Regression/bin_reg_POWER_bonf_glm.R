# Simulation of the Power in a regression model with the GLM and the Bonferroni method
...
sim_mmm_bin_power_bonf_glm <- function(nsim, 
                        ntrt, 
                        pi0, 
                        nep_h0, 
                        nep_ha, 
...
system.time(sim_mmm_bin_power_bonf_glm_res <- do.call(rbind,apply(as.matrix(simdat), 1, 
                                                   function(x){sim_mmm_bin_power_bonf_glm(nsim=unname(x[1]),ntrt=unname(x[2]),pi0=unname(x[3]), 
                                                                           nep_h0=unname(x[4]),nep_ha=unname(x[5]),slope=unname(x[6]),
                                                                           size=unname(x[7]),corr=unname(x[8]))})))

write.csv(sim_mmm_bin_power_bonf_glm_res, ".\\intermediate_results\\bin_reg_POWER_bonf_glm_1.csv", row.names = FALSE)
