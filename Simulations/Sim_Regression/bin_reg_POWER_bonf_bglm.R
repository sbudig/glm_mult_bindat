# Simulation of the Power in a regression model with the BayesGLM and the Bonferroni method
...
# using the bonferroni approach. The parameters used and the corresponding 
# results to obtain the Power are returned in a data frame. 
sim_mmm_bin_power_bonf_bglm <- function(nsim, 
                        ntrt, 
                        pi0, 
                        nep_h0, 
                        nep_ha, 
...
system.time(sim_mmm_bin_power_bonf_bglm_res <- do.call(rbind,apply(as.matrix(simdat), 1, 
                                                   function(x){sim_mmm_bin_power_bonf_bglm(nsim=unname(x[1]),ntrt=unname(x[2]),pi0=unname(x[3]), 
                                                                           nep_h0=unname(x[4]),nep_ha=unname(x[5]),slope=unname(x[6]),
                                                                           size=unname(x[7]),corr=unname(x[8]))})))

write.csv(sim_mmm_bin_power_bonf_bglm_res, ".\\intermediate_results\\bin_reg_POWER_bonf_bglm_1.csv", row.names = FALSE)
