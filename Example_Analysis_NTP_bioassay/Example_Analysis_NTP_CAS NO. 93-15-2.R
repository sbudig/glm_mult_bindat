...
#setwd("...\Code_and_Data\Example_Analysis_NTP_bioassay")
...
## p-values for each model are extracted from summary 
p_vec_glm <- lapply(fitlist_glm, function(x) summary(x)$coefficients[c(2:4),4])
p_vec_glm <- as.vector(unlist(p_vec_glm))
...
## p-values for each model are extracted from summary 
p_vec_bglm <- lapply(fitlist_bglm, function(x) summary(x)$coefficients[c(2:4),4])
p_vec_bglm <- as.vector(unlist(p_vec_bglm))
...
mmm_glht_glm <- glht(mmm_glm, linfct = mlf(mcp(dose = "Dunnett")), alternative = "two.sided")
p_adj_glm_mmm <- summary(mmm_glht_glm)
...
mmm_glht_bglm <- glht(mmm_bglm, linfct = mlf(mcp(dose = "Dunnett")), alternative = "two.sided")
p_adj_bglm_mmm <- summary(mmm_glht_bglm)
...
## extract the p-values of interest with the summary function and store into vector
l_pvalues_glm <- lapply(fitlist_glm, function(x) summary(x)$coefficients[c(2:4),4])
v_pvalues_glm <- as.vector(unlist(l_pvalues_glm))
...
## extract the p-values of interest with the summary function and store into vector
l_pvalues_bglm <- lapply(fitlist_bglm, function(x) summary(x)$coefficients[c(2:4),4])
v_pvalues_bglm <- as.vector(unlist(l_pvalues_bglm))
