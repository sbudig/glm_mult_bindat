# Simulation of the FWER in a regression model with the BayesGLM and the Bonferroni method
...
sim_mmm_bin_fwer_bonf_bglm <- function(nsim, 
                        ntrt, 
                        pis, 
                        nresp, 
                        size, 
...
system.time(sim_mmm_bin_fwer_bonf_bglm_res <- do.call(rbind,apply(as.matrix(simdat_reg_FWER), 1, 
                                                   function(x){sim_mmm_bin_fwer_bonf_bglm(nsim=unname(x[1]),pis=unname(x[2]),nresp=unname(x[3]), 
                                                                           ntrt=unname(x[4]),size=unname(x[5]),corr=unname(x[6]))})))

write.csv(sim_mmm_bin_fwer_bonf_bglm_res, ".\\intermediate_results\\bin_reg_FWER_bonf_bglm_1.csv", row.names = FALSE)
