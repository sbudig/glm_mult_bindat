# Simulation of the FWER in a regression model with the BayesGLM and the bootstrap method
...
# using the bootstrap approach. The parameters used and the corresponding results to obtain 
# the FWER are returned in a data frame. 
sim_mmm_bin_fwer_bt_glm <- function(nsim, 
                        ntrt, 
                        pis, 
                        nresp, 
                        size, 
...
system.time(sim_mmm_bin_fwer_bt_glm_res <- do.call(rbind,apply(as.matrix(simdat_reg_FWER), 1, 
                                                   function(x){sim_mmm_bin_fwer_bt_glm(nsim=unname(x[1]),pis=unname(x[2]),nresp=unname(x[3]), 
                                                                           ntrt=unname(x[4]),size=unname(x[5]),corr=unname(x[6]), 
                                                                           nboot=unname(x[7]))})))

write.csv(sim_mmm_bin_fwer_bt_glm_res, ".\\intermediate_results\\bin_reg_FWER_bt_bglm_1.csv", row.names = FALSE)
