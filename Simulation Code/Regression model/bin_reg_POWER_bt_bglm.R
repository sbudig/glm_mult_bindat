...
sim_mmm_bin <- function(nsim, ntrt, pi0, nep_h0, nep_ha, slope, size, corr, nboot, di = c(0,2,5,10), alpha=0.05){
  ...
  for(i in 1:nsim)
    ...
    tryCatch({
      ...
      for (b in 1:nboot) {
        ...
        RSDAT <- apply(dplyr::select(dati, starts_with("s")), 2, sample)
        RFDAT <- size-RSDAT
...
write.csv(sim_mmm_bin_res, "bin_reg_POWER_bt_bglm_1.csv", row.names = FALSE)
