...
# 
# # Generate correlated Binomial data Function  -----------------------------
# 
# simind_corbin_cop <- function(ntrt, mutrt, cormat, size){
#   
...
#   
#   # define the factors for trt ans subgroups accordingly
#   ftrt <- factor(rep(ltrt, times=nvectrt), levels=ltrt)
#   
#   # sample multivariate binary observations using the copula package
#   probtrt <- expit(mutrt)
#   
#   NORMCOP <- normalCopula(param=corvec, dim = noend, dispstr = "un")
#   
#   MAR <- rep("binom", times=noend)
#   
#   DAT <- NULL
#   
#   for(i in 1:length(ntrt)){
#     # define the copula distribution with parameters of current group i:
#     MVBi <- mvdc(copula=NORMCOP, margins=MAR, paramMargins=paralistbin(size=size, prob=probtrt[i,]))
#     
#     # sample data from this copula distribution
#     DATi <- rMvdc(n=nvectrt[i], mvdc=MVBi)
#     DAT <- rbind(DAT, DATi)
#   }
#   
#   SDAT <- DAT
...
sim_mmm_bin <- function(nsim, di = c(0,2,5,10), ntrt, pi0, size, alpha=0.05){

  
NTRT <- rep(ntrt, times = length(di))
names(NTRT) <- di

mutrt <- mi0ed(mi=logit(pi0), ndos=length(di), nresp=3)
MUTRT <- t(mutrt)
...
results <- data.frame(cor_s_s1s2 = numeric(), cor_s_s1s3 = numeric(), cor_s_s2s3 = numeric(),
                      cor_m_s1s2 = numeric(), cor_m_s1s3 = numeric(), cor_m_s2s3 = numeric(),
                      cor_p_s1s2 = numeric(), cor_p_s1s3 = numeric(), cor_p_s2s3 = numeric(),
                      t1 = numeric(), t2 = numeric(), t3 = numeric(),
                      ntrt = numeric(), pi0 = numeric(), nsim = numeric())
...
for(i in 1:nsim)
{
  ...
  class(FITLIST) <- "mmm"
    ...
    temp <- data.frame(cor_s_s1s2 =  VCOV[1,2], cor_s_s1s3 =  VCOV[1,3], cor_s_s2s3 =  VCOV[2,3],
                          cor_m_s1s2 = cor_mmm[1,2], cor_m_s1s3 = cor_mmm[1,3], cor_m_s2s3 = cor_mmm[2,3],
                          cor_p_s1s2 = cor_pear[1,2], cor_p_s1s3 = cor_pear[1,3], cor_p_s2s3 = cor_pear[2,3],
                          t1 =  as.numeric(tstat[1]), t2 = as.numeric(tstat[2]), t3 = as.numeric(tstat[3]),
                          ntrt = ntrt, pi0 = pi0, nsim = nsim)
...
write.csv(sim_mmm_bin_res, "bin_check_correlation_endpoints_1.csv", row.names = FALSE)
